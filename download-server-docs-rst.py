...
def h1_underline(header_text):
    ul_length = len(header_text)
    h1_underline = '*' * ul_length
    return h1_underline

def h2_underline(header_text):
    ul_length = len(header_text)
    h2_underline = '=' * ul_length
    return h2_underline
...
# Remove markdown files before re-downloading the current/up-to-date ones.
...
# Remove the subsections folder and all pages within, which provides all the
# Diataxis scaffolding (fully generated, not downloaded), then remake folder.
...
with open(csv_file_list, newline='', encoding='utf-8') as filelist:
    ...
    for index, row in enumerate(reader):
        ...
        if response.status_code == 200:
            # Create path for saving file (based on Diataxis). Since we are 
            # creating landing pages separately, we don't want to save any file
            # that has a page type of "landing page". 
            if page_type != "landing page":
                doc_filename = f"{page_type}/{page_slug}.md"
            else:
                doc_filename = f"{page_slug}.rst"
...
print("Searching landing pages: replacing reference numbers")
for file in landing_pages:
    ...
    if new_content != file_content:
        with open(file, 'w') as f:
            f.write(new_content)
print("Finished replacing reference numbers in landing pages")
...
with open(csv_file_list, newline='') as filelist:
    ...
    for index, row in enumerate(reader):
        ...
        if subsection_name != prev_subsection:
            ...
            h1_underline = heading_underline(h1_header)
                
            if subsubsection_name != '--':
                list_contents += f"\n\n**{subsubsection_name}**\n\n"

            list_contents += f"* :ref:`{page_title} <{page_slug}>`\n"
            toc_contents += f"    {page_title} <../{page_type}/{page_slug}.rst>\n"
    ...
    with open(csv_file_list, newline='') as filelist:
        ...
        for index, row in enumerate(reader):
            ...
            if (page_type == folder) and (subsection_name != prev_subsection):
                ...
                subsection = subsection_name.lower()
                subsection = subsection.replace(' ','-')
                h2_underline = heading_underline(subsection_name)

                list_contents += f"\n\n{subsection_name}\n"
                list_contents += h2_underline
                list_contents += f"\n\n.. include:: <subsections/{page_type}-{subsection}.rst>\n\n"                    
                list_contents += f"    :start-line: 5\n"

...
The reason for this is that `multipath.conf` is copied to the RAM disk, and is integral to determining the available devices to map via its [`denylist`](https://documentation.ubuntu.com/server/reference/glossary/#term-denylist) and `devices` sections. 
```

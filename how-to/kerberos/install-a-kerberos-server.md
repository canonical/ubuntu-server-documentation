...
Before installing the Kerberos server, a **properly configured [DNS](https://documentation.ubuntu.com/server/reference/glossary/#term-DNS) server** is needed for your domain. Since the Kerberos realm (by convention) matches the domain name, this section uses the `EXAMPLE.COM` domain configured in the primary server section of the {ref}`DNS documentation <install-dns>`.

 ...
 * **ZFS**: it is a next generation filesystem that supports many advanced storage technologies such as volume management, snapshots, checksumming, compression and [deduplication](https://documentation.ubuntu.com/server/reference/glossary/#term-deduplication), replication and more. The Docker storage driver is called `zfs`.

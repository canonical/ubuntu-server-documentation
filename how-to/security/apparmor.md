...
[AppArmor](https://apparmor.net/) is an easy-to-use Linux Security Module implementation that restricts applications' capabilities and permissions with **profiles** that are set per-program. It provides mandatory access control (MAC) to supplement the more traditional UNIX model of [discretionary access control (DAC)](https://documentation.ubuntu.com/server/reference/glossary/#term-DAC).
...
You will see in [`dmesg`](https://documentation.ubuntu.com/server/reference/glossary/#term-dmesg) (and any log that collects kernel messages) if you have hit a **deny**.

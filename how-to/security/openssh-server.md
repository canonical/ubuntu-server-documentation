...
If needed, you can generate the key using the **RSA Algorithm** by using `-t rsa -b 4096`.

During creation, you will be prompted for a key password, which you would enter to use that key (every time, or instead using an [ssh agent](#ssh-agent).

By default, the public key is saved in the file `~/.ssh/id_ed25519.pub`, while `~/.ssh/id_ed25519` is the private key.
To allow login for a user via key, append the content of `id_ed25519.pub` to `target_machine:~/.ssh/authorized_keys` by running:
...
To troubleshoot this, have a look at the `target_machine`'s journal:

```bash
sudo journalctl -eu sshd.service
```


## Connection multiplexing

To reduce connection times, multiple SSH sessions to the same `target_machine` can work over one {term}`TCP` connection.

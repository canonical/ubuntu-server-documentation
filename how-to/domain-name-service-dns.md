...
[See dig](domain-name-service-dns.md) for information on testing a caching DNS server.
...
Now, you can add DNS records to the bottom of the zone file. See [Common Record Types](domain-name-service-dns.md) for details.
...
> **Note**:
> The default directory for non-authoritative zone files is `/var/cache/bind/`. This directory is also configured in AppArmor to allow the named daemon to write to it. See this page for [more information on AppArmor](apparmor.md).

## Testing your setup

### resolv.conf

The first step in testing BIND9 is to add the nameserver's IP address to a **hosts resolver**. The Primary nameserver should be configured as well as another host to double check things. Refer to [DNS client configuration](../explanation/configuring-networks.md) for details on adding nameserver addresses to your network clients. In the end your `nameserver` line in `/etc/resolv.conf` should be pointing at `127.0.0.53` and you should have a `search` parameter for your domain. Something like this:

...
'Yes' should be selected to enable `kdump-tools`.

If you ever need to manually enable the functionality, you can use the `dpkg-reconfigure kdump-tools` command and answer 'Yes'. You can also edit `/etc/default/kdump-tools` to enable `kdump` by including the following line:

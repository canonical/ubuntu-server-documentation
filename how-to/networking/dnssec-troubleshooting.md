...
DNSSEC can be very complicated, and bind9 takes away a lot of that complication for the common cases, with the *default* DNSSEC policy. But sometimes things just don't work out.
...
One of the more versatile tools to aid in DNS troubleshooting in general is `dig`. With this tool, we have direct control over most of the DNS flags set in queries, and can inspect responses in detail.
...
In all these cases, however, `dig` itself never performed any DNSSEC validation. It just presented the results that the validating resolver provided. The DNSSEC validation, when requested, was always done by the validating resolver. And sometimes no validation at all was being done (via the `+cd` flag). To perform validation ourselves, we have to use something else.

## Digging a bit deeper: `delv`

The `delv` tool is very similar to `dig`, and can perform the same DNS queries, but with a crucial difference: it can also validate DNSSEC. This is very useful in troubleshooting, because instead of returning a generic SERVFAIL error when something goes wrong with DNSSEC validation, it can tell us actually what was wrong in more detail.

But to bring the responsability of doing DNSSEC validation to the tool itself, we have to use the `+cd` flag in our queries, to tell the resolver to not attempt that validation. Otherwise we will just get back a generic SERVFAIL error:
...
An RRSIG record (a digital signatur of a resource record) has a validity. For example, this record:
...
If the DNSSEC validator has a wrong clock, outside of the validity range, the DNSSEC validation will fail. For example, with the clock incorrectly set to before the beginning of the validity period, `delv` will complain like this:

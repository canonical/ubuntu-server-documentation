...
A very simple `/etc/multipath.conf` file exists, as explained in [the configuration overview](configuring-device-mapper-multipathing.md). All attributes not declared in `multipath.conf` are taken from the `multipath-tools` internal database and its internal blacklist.

...
Libvirt and QEMU allow the CPU to be expressed in ways that ease migration between hosts in heterogeneous infrastructures. While a libvirt VM configured with `host-passthrough` or `maximum` (the equivalent QEMU `-cpu host` and `-cpu max`) gets most out of the hardware, it exposes the guests to all available host capabilities. Therefore, migration to another host is much more restricted, because it needs to be able to present the very same environment to the guest. To do so it needs a clear definition that helps to ensure both sides of the migration can present the same. Therefore it is preferred to specifying the CPU capabilities as a baseline CPU model +/- a list of features to ensure compatibility. In QEMU, it comes down to this example `-cpu` argument:

...
Libvirt and QEMU allow the CPU to be expressed in ways that ease migration between hosts in heterogeneous infrastructures. While a libvirt VM configured with `host-passthrough` or `max` (the equivalent QEMU `-cpu host` and `-cpu max`) takes the most out of the hardware and kernel hypervisor, it exposes the guests to all available host capabilities. Therefore, migration to another host is much less transparent to the guests (by exposing new capabilities or deprecating existing features), and this can break the guests after migration. Instead of specifying the CPU capabilities in such manner, a CPU can be expressed as a baseline CPU model +/- a list of features. In QEMU, it comes down to this example `-cpu` argument:

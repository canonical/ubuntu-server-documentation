...
Versioned machine types keep the virtual platform stable, but they do not control the CPU that the guest sees. The CPU is exposed separately, and to maintain a stable CPU definition on both the source and destination hosts, you should use a CPU baseline and named features. This is the second mechanism that makes migration across heterogeneous hosts possible.  
...
If this is CPU definition list too complex to build and maintain, libvirt can help to build this for you if the `host-model` CPU mode is used in the guest domain definition.
This detects the current CPU with all its features, automatically expressed as baseline CPU model +/- a list of features.
...
If you prefer more control, you can provide the exact CPU specification:
...
While the first approach of just using `host-passthrough` and even `host-model` seems easier and more appealing, it is not suitable for heterogeneous infrastructure. To retain full control over migration compatibility across every host in the migration pool a CPU should be modeled.
...
This block is the lowest common denominator CPU for the pool. Copy it into the domain XML of the VMs you intend to migrate, so that these VMs see the same CPU regardless of which host they run on.

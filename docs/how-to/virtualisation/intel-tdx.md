...
(intel-tdx)=
# Confidential Computing with Intel
...
To enable Intel TDX on the host, ensure TDX support is enabled in the system firmware/BIOS settings. 
...
You must also add the following kernel parameters to enable TDX:

```
nohibernate kvm_intel.tdx=1
```

These parameters can be added to your bootloader configuration (e.g., in `/etc/default/grub` by appending them to `GRUB_CMDLINE_LINUX_DEFAULT`, then running `sudo update-grub`).
...
To check if the host supports Intel TDX:
...
Alternatively, check for hardware capabilities required for Intel TDX:
...
Install the required virtualization components to run TDX-enabled VMs:
...
Launch a TDX-enabled VM using QEMU with the prepared image:
...
Once the VM is launched, verify that TDX is active by checking the kernel logs:

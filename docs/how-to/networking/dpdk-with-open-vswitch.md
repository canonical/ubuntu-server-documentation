...
```{note}
You need to assign devices to DPDK-compatible drivers before restarting --  see the DPDK section on [unassigning the default kernel drivers](https://documentation.ubuntu.com/server/network-dpdk//#heading--unassign-default-kernel-drivers).
```
